...
class StockLocationInherit(models.Model):
    ...
    def _compute_quantity_rack(self):
        quants = self.env['stock.quant'].search([('location_id','in',self.ids),('location_id.usage','=','internal')])
        for rec in self:
            if rec.usage !='internal':
                rec.percentage_ocupado = 0
                rec.qty_rack_free = 0
                continue
            quants_loc = quants.filtered_domain([('location_id', '=', rec.id)])
            total_rack = rec.total_rack
            percentage_ocupado = 0
            qty_rack_free = 0
            if quants_loc and total_rack:
                quantity = sum(quants_loc.mapped('quantity'))
